...
const debug_log = require('debug')('express-mysql-session:log');
const debug_error = require('debug')('express-mysql-session:error');
const deprecate = require('depd')('express-mysql-session');
...
module.exports = function (session) {
    ...
    MySQLStore.prototype.set = function (session_id, data, cb) {
        ...
        var sql = 'INSERT INTO ?? (??, ??, ??, ??) VALUES (?, ?, ?, UNIX_TIMESTAMP() * -1) ON DUPLICATE KEY UPDATE ?? = VALUES(??), ?? = VALUES(??)';
        ...
        var params = [
            ...
            this.options.schema.columnNames.expires,
            this.options.schema.columnNames.data,
            this.options.schema.columnNames.recency,
            session_id,
            expires,
            data,
            this.options.schema.columnNames.expires,
